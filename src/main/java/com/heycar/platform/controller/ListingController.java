...
public class ListingController {
    ...
    @PostMapping(value = "/upload_csv/{dealer_id}",consumes = "text/csv")
    public ResponseEntity test(HttpServletRequest request,@PathVariable("dealer_id") String dealerId,
                               @Valid @RequestBody ListingList listing){
        ...
        for(ListingDocument doc : listingDocument) {
            headers.add("Location", request.getRequestURL().append("/").append(doc.getDealerIdCode()).toString());
        }
    ...
    public ResponseEntity addListing(HttpServletRequest request,
